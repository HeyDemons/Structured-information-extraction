...
def do_parse(
    ...
    #     pdf_file_names (list[str]): 与输入字节流一一对应的 PDF 文件名列表，用于生成本地输出目录结构。
    #     pdf_bytes_list (list[bytes]): 待解析的 PDF 二进制内容列表。
    #     p_lang_list (list[str]): 每个 PDF 的语言配置列表，默认使用 'ch'（中文），用于选择合适的 OCR/解析流程。
    #     backend (str): 解析后端，可选 "pipeline"、"vlm-*"、"hybrid-*”；决定整体处理流程与所用模型。
    #     parse_method (str): 解析策略，"auto" 会根据后端自动决定具体执行路径。
    #     formula_enable (bool): 是否启用公式识别，影响中间结果的数学内容提取。
    #     table_enable (bool): 是否启用表格识别，影响表格结构化输出。
    #     server_url (str | None): 当使用远程 VLM 后端时的服务地址。
    #     f_draw_layout_bbox (bool): 是否在输出中绘制版面布局的边界框图像。
    #     f_draw_span_bbox (bool): 是否绘制文本 span 的边界框，仅对部分后端生效。
    #     f_dump_md (bool): 是否输出 Markdown 文件。
    #     f_dump_middle_json (bool): 是否导出中间 JSON 结果，便于调试或二次处理。
    #     f_dump_model_output (bool): 是否保存底层模型的原始输出。
    #     f_dump_orig_pdf (bool): 是否拷贝原始 PDF 到输出目录。
    #     f_dump_content_list (bool): 是否导出内容列表文件（如段落/块索引）。
    #     f_make_md_mode (MakeMode): Markdown 生成模式，用于控制多模态内容的布局策略。
    #     start_page_id (int): 解析起始页（0 基），可用于截取部分页面。
    #     end_page_id (int | None): 解析终止页（不含），None 表示一直处理到文档末尾。
    # '''
    ...
    md_collect_dir = Path(output_dir) / "md"
    md_collect_dir.mkdir(parents=True, exist_ok=True)
    collect_md_writer = FileBasedDataWriter(str(md_collect_dir))
    ...
    if backend == "pipeline":
        ...
        for idx, model_list in enumerate(infer_results):
            ...
            _process_output(
                ...
                md_writer, collect_md_writer,
                f_draw_layout_bbox, f_draw_span_bbox, f_dump_orig_pdf,
...
def _process_output(
    ...
    if f_dump_md:
        ...
        if md_collect_writer is not None:
            md_collect_writer.write_string(
                f"{pdf_file_name}.md",
                md_content_str,
            )
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        help="包含待解析 PDF/图片的目录，默认读取环境变量 MINERU_INPUT_DIR 或 demo 下的 pdfs 目录",
    )
    parser.add_argument(
        "--output-dir",
        type=str,
        default=default_output_dir,
        help="解析结果输出目录，默认读取环境变量 MINERU_OUTPUT_DIR 或 demo 下的 output 目录",
    )
    args = parser.parse_args()
    pdf_files_dir = args.input_dir
    if not Path(pdf_files_dir).exists():
        raise FileNotFoundError(f"指定的输入目录不存在: {pdf_files_dir}")
